...
public:
    ...
    void onConnectionStatusChanged(int connection_status) {
        ...
        cb->_paramVal[0] = INT_TO_JSVAL(cx, connection_status);
        cb->_paramLen = 1;
        cb->schedule();
    }
    void onScoreSubmitted(const std::string &leaderboard_name, int score, bool alltime, bool week, bool day) {
        ...
        cb->_paramVal[1] = INT_TO_JSVAL(cx, score);
        cb->_paramVal[2] = BOOL_TO_JSVAL(cx, alltime);
        cb->_paramVal[3] = BOOL_TO_JSVAL(cx, week);
        cb->_paramVal[4] = BOOL_TO_JSVAL(cx, day);
        cb->_paramLen = 5;
        cb->schedule();
    }
    ...
    void onIncrementalAchievementStep(const std::string &achievement_name, int step) {
        ...
        cb->_paramVal[0] = std_string_to_jsval(cx, achievement_name);
        cb->_paramVal[1] = INT_TO_JSVAL(cx, step);
        cb->_paramLen = 2;
        cb->schedule();
    }
    void onAchievementUnlocked(const std::string &achievement_name, bool newly) {
        ...
        cb->_paramVal[1] = BOOL_TO_JSVAL(cx, newly);
        cb->_paramLen = 2;
        cb->schedule();
    }
